"""
This module is adapted from https://github.com/lm-sys/FastChat
...
class Conversation:
    ...
    def get_prompt(self) -> str:
        ...
        if self.sep_style == SeparatorStyle.ADD_COLON_SINGLE:
            ...
            if self.system_message:
                ...
                ret = "[INST] "
            for i, (role, message, _) in enumerate(self.messages):
                tag = self.roles[i % 2]
                if message:
                    if i == 0:
                        ret += message + " "
    ...
    def get_separate_prompt_with_to_loss(self):
        system_prompt = self.system_template.format(system_message=self.system_message)
        separate_prompts = []
        if self.sep_style == SeparatorStyle.LLAMA2:
            ...
            if self.system_message:
                ...
                ret = "[INST] "
            for i, (role, message, to_loss) in enumerate(self.messages):

                # tag = self.roles[i % 2]
                tag = role
                if message:
                    if i == 0:
                        # Should we consider this because the first turn is usually user
                        if to_loss:
                            separate_prompts.append((ret, False))
                            separate_prompts.append((message, True))
                            ret = " "
                        else:
                            ret += message
                            separate_prompts.append((ret, False))
                        ...
                        if to_loss:
                            ...
                            separate_prompts.append((ret, False))
                            if seps[i%2] == " </s><s>":
                                # calculate loss on </s> so that the model can learn to stop
                                separate_prompts.append((" " + message + " </s>", True))
                                ret = "<s>"
                            else:
                                separate_prompts.append((" " + message, True))
                                ret = seps[i % 2] # " "
                        else:
                            # TODO: check more to see if this is correct
                            ret += tag + " " + message
                            separate_prompts.append((ret, False))
                            ret = seps[i % 2]
                        # ret += tag + " " + message + seps[i % 2]
            ...
            # post process
            # We need to merge the prompts with the same loss,
            # This is to make the tokenized output consistent with the original one.
            # Because a space inside a string is tokenized different with a space in
            # the heading of a string.
            # TODO: Need more test to check if this is correct.
            processed_separate_prompts = []
            current_loss = separate_prompts[0][1]
            current_prompt = ""
            for i, (prompt, to_loss) in enumerate(separate_prompts):
                if current_loss == to_loss:
                    current_prompt += prompt
                else:
                    processed_separate_prompts.append((current_prompt, current_loss))
                    current_prompt = prompt
                    current_loss = separate_prompts[i][1]
            if current_prompt != "":
                processed_separate_prompts.append((current_prompt, current_loss))

            return processed_separate_prompts
        else:
            raise NotImplementedError(f"Invalid style: {self.sep_style}")

    def hightlight_with_to_loss(self):
        from termcolor import colored
        all_prompt = ""
        for prompt, to_loss in self.get_separate_prompt_with_to_loss():
            if to_loss:
                all_prompt += colored(prompt, "red")
            else:
                all_prompt += prompt
        return all_prompt
    ...
    def append_message(self, role: str, message: str, to_loss=False):
        """Append a new message."""
        self.messages.append([role, message, to_loss])
...
if __name__ == "__main__":
    ...
    conv.append_message(conv.roles[1], "Hi!", True)
    conv.append_message(conv.roles[0], " How are you?")
    conv.append_message(conv.roles[1], "I'm fine, thanks! ", True)
    conv.append_message(conv.roles[0], "What is your homework?")
